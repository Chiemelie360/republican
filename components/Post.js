...
function Post({ id, post, postPage }) {
    ...
    useEffect(() => setLiked(likes.findIndex((like) => like.id === session?.
    user?.uid) !== -1),
    [likes]
    );
...
<div className="inline-block group">
        <h4 className={`font-bold text-[15px] sm:text-base
         ...
         ${!postPage && "inline-block"}`}>{post?.username}</h4>
        <span className={`text-sm sm:text-[i5px]
         ${!postPage && "ml-1.5"}`} 
         >@{post?.tag}
         </span>
</div>{" "}
·{" "}
<spam className="hover:underline text-sm sm:text-[15px]">
